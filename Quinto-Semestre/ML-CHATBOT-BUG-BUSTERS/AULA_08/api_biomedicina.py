...
Exemplo de requisição (curl):
    curl -X POST "http://localhost:8000/predict" \
         ...
         -d '{"nome":"João","idade":45,"glicose":120,"pressao":130,"imc":25,"colesterol":200}'
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "probabilidade": 0.95,
                "explicacao": "Paciente com múltiplos fatores de risco (glicose elevada, hipertensão, IMC alto)"
            }
        }
...
async def carregar_modelo():
    ...
    Executa uma única vez quando o servidor inicia.
    """
...
async def predict(paciente: PacienteInput):
    """
    Endpoint principal: faz o diagnóstico de risco para um paciente.
    
    Recebe dados do paciente e retorna a classificação de risco (Baixo/Médio/Alto).
    
    **Parâmetros:**
    - nome: string com o nome do paciente
    - idade: inteiro entre 18 e 120 anos
    - glicose: float entre 60 e 350 mg/dL
    - pressao: float entre 80 e 220 mmHg
    - imc: float entre 15 e 55 kg/m²
    - colesterol: float entre 100 e 400 mg/dL
    
    **Retorna:**
    - nome: nome do paciente
    - idade: idade fornecida
    - risco_codigo: 0 (Baixo), 1 (Médio), 2 (Alto)
    - risco_classificacao: descrição do risco
    - probabilidade: confiança da predição (0.0 a 1.0)
    - explicacao: resumo dos fatores de risco
    """
    ...
    if modelo is None:
        ...
        raise HTTPException(
            status_code=503,
            detail="Modelo de ML não foi carregado. Tente novamente mais tarde."
        )
    
    try:
        # 1. EXTRAIR DADOS NA ORDEM CORRETA (mesma ordem do treino)
        features_valores = [
            paciente.idade,
            paciente.glicose,
            paciente.pressao,
        ...
        X = np.array([features_valores])  # shape (1, 5)
        X_normalizado = scaler.transform(X)  # normaliza usando os parametros do treino
        
        # 3. FAZER PREDIÇÃO
        classe_predita = modelo.predict(X_normalizado)[0]  # retorna classe: 0, 1 ou 2
        probabilidades = modelo.predict_proba(X_normalizado)[0]  # retorna [prob_0, prob_1, prob_2]
        probabilidade_max = float(probabilidades[int(classe_predita)])  # confianca da predicao
        
        # 4. MAPEAR CLASSE PARA RÓTULO
        risco_classificacao = LABELS_RISCO[int(classe_predita)]
        
        # 5. GERAR EXPLICAÇÃO BASEADA NOS FATORES DE RISCO
        fatores_risco = []
        ...
        # 7. LOG DA PREDIÇÃO
        logger.info(f"Predição realizada: {paciente.nome} -> Risco {risco_classificacao} ({probabilidade_max:.2%})")
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao processar predição: {str(e)}")
